...
#     bash board.sh cue [hw:X]      the headphones out of the USB dongle (default) or another card
...
# AUDIO_OUT and CUE_OUT pick the cards for master and headphones at start.
set -e
IF="${BOARD_IF:-en0}"
...
case "${1:-}" in
    ...
    start)   ssh $O "root@$H" "PERIOD_MIN=${PERIOD_MIN:-0} bash /root/az01-run.sh && bash /root/az01-mirror.sh && bash /root/az01-cool.sh && (sleep 45; OUT=${AUDIO_OUT:-hw:HDMI} bash /root/az01-audio.sh; [ -n '${CUE_OUT:-}' ] && bash /root/az01-audio.sh cue ${CUE_OUT:-}) > /dev/null 2>&1 &" ;;
    ...
    cue)     shift; ssh $O "root@$H" "bash /root/az01-audio.sh cue ${1:-hw:Device}" ;;
